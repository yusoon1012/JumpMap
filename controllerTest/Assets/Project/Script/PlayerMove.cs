...
public class PlayerMove : MonoBehaviour
{
    ...
    private void GetInput()
    {
        ...
        fire = player.GetButtonDown("Player Fire");
    ...
    void Update()
    {
            animator.SetBool("isAttack", fire);
        ...
        if (!isGround)
        {
            ...
            //Debug.Log(airbornTimer);
            
        }
        if(airbornTimer>=airbornRate)
        {
            Debug.Log("낙하");
            fallDamage = true;
            airbornRate = 1f;
        }
        if(fallDamage==true&&isGround==true)
        {
            int motorIndex = 0; // the first motor
            float motorLevel = 0.05f; // full motor speed
            float duration = 0.2f; // 2 seconds
        ...
        //float zSpeed = moveVertical * speed;
        //Vector3 movment = new Vector3(xSpeed, 0f, zSpeed);
        ////playerRigid.AddForce(movment * speed);
        ...
        //transform.LookAt(transform.position + movment);
